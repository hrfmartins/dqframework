...
from dqframework.validators import has_max_length


def test_has_max_length():
    df = pl.DataFrame({"a": ["a", "ab", "abc"], "b": ["abcd", "abcde", "abcdef"]})

    correct, incorrect = has_max_length(df, "a", 1)
...
def test_has_max_length_with_no_incorrects():
    ...
    correct, incorrect = has_max_length(df, "a", 3)
...
def test_has_max_length_with_all_incorrects():
    ...
    correct, incorrect = has_max_length(df, "a", 0)
